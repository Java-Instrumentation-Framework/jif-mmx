...
import edu.mbl.jif.ps.orient.color.aeten.Colors.HSx;
...
public class HSxSelector extends JComponent {
	...
	public HSxSelector(final HSx hsx, final HueSelector hueSelector) {
		this(hsx, hueSelector, true);
	}

	public HSxSelector(final HSx hsx, final HueSelector hueSelector, boolean extentChroma) {
		this.hsx = hsx;
	...
	private double getSaturation(int x, int y) {
		if (!extentChroma && (hsx == HSx.HSI || hsx == HSx.HSYp)) { return hsx.getSaturationFromChroma((double) x / (double) getWidth(), hue, (double) (getHeight() - y) / (double) getHeight()); }
		return (double) x / (double) getWidth();
	}

	private int getX(double[] rgb) {
		if (!extentChroma && (hsx == HSx.HSI || hsx == HSx.HSYp)) { return (int) (hsx.getChroma(rgb) * getWidth()); }
