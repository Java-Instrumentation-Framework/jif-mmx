package edu.mbl.jif.ps.orient.orient.measure;
import java.awt.*;
...
public class Analyzer implements PlugInFilter, Measurements {
	...
	private static boolean showMin = true;
      ...
      static {
		...
		setResultsTable(Analyzer.getResultsTable());
      }
	public Analyzer() {
		rt = systemRT;
	...
	*/
	public synchronized static boolean resetCounter() {
		TextPanel tp = Analyzer.textPanel;
                    // = this.IJ.isResultsWindow()?IJ.getTextPanel():null;
	...
	public static void setResultsTable(ResultsTable rt) {
		TextPanel tp = Analyzer.textPanel;
                    // = IJ.isResultsWindow()?IJ.getTextPanel():null;
		if (tp!=null)
			tp.clear();
		if (rt==null)
			rt = new ResultsTable();
