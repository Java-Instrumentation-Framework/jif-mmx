...
public class ColorWheelPanel
    ...
    public ColorWheelPanel() {
        ...
        brightnessSlider.setBounds( 230, 0, 25, 110 );
        ...
        saturationSlider.setBounds( 230, 118, 25, 110 );
        ...
        useWebColors = new JCheckBox( getLabel( "Xoetrope.webSafeColors", "Use web safe colors" ));
        ...
        decimalRGB = new JCheckBox( getLabel("Xoetrope.decimalRGB", "Decimal RGB" ));
        ...
        hueEdit.getDocument().addDocumentListener( new ColorDocumentListener( hueEdit ));
        ...
        satEdit.getDocument().addDocumentListener( new ColorDocumentListener( satEdit ));
        
        JLabel satLabel = new JLabel( getLabel( "Xoetrope.saturation", "Saturation" )+ " %" );
        ...
        brightEdit.getDocument().addDocumentListener( new ColorDocumentListener( brightEdit ));
        
        JLabel brightLabel = new JLabel( getLabel( "Xoetrope.brightness", "Brightness" ) + " %" );
     ...
     * @param disp true to display the color scheme markers.
     */
     ...
     * <li>2 for a soft-contrasting color scheme: SOFT_CONTRAST_SCHEME</li>
     * <li>3 for a double contrasting color scheme: DOUBLE_CONTRAST_SCHEME</li>
     ...
     * @param pt the point within the inner circle
     */
    ...
    public void setColor( Color c ) {
        ...
        baseColorLabel.setBackground( new Color( chooserColor.R, chooserColor.G, chooserColor.B ));
        ...
        if ( decimalRGB.isSelected() ) {
            ...
            colorStr = " " + Integer.toString( c.getRed() ) + "." + Integer.toString( c.getGreen() ) + "." +
                    Integer.toString( c.getBlue() ) ;
        } else {
            // Output HEX values
            colorStr = " " + ModelColor.toHexString( c.getRed() ) + ModelColor.toHexString( c.getGreen() ) +
            ...
            if ( hex.length() == 0 )
                ...
                try {
                    setColor( new Color( ModelColor.hex2dec( hex.substring( 0, 2 ) ),
                            ModelColor.hex2dec( hex.substring( 2, 4 ) ),
                            ModelColor.hex2dec( hex.substring( 4, 6 ) ) ) );
                } catch ( NumberFormatException nfe ) {
                    setColor( Color.red );
            ...
            if ( borderCircle.contains( pt )) {
                ...
                if ( !moveHue( pt )) {
                    if ( outerCircle.contains( pt )) {
                        ...
                        double dr = Math.pow( (double)( dx * dx + dy * dy ), 0.5 );
    ...
    public void stateChanged( ChangeEvent e ) {
        ...
        if ( hasChooser ) {
            getColorSelectionModel().setSelectedColor(
                    new Color( chooserColor.getRed(),
                    chooserColor.getGreen(),
...
//      int pathIdx = 0;
     ...
     * @param p the location to display the popup
     */
    private void showSystemColorList( Point p ) {
        JPopupMenu popupMenu = new JPopupMenu();
        String[] systemColors = { "activeCaption", "desktop", "activeCaptionText"
                , "activeCaptionBorder", "inactiveCaption", "inactiveCaptionText", "inactiveCaptionBorder"
                , "window", "windowBorder", "windowText", "menu"
                , "menuText", "text", "textText", "textHighlight", "textHighlightText"
                , "textInactiveText", "control", "controlText", "controlHighlight"
                , "controlLtHighlight", "controlShadow", "controlDkShadow", "scrollbar"
                , "info", "infoText", "white", "lightGray"
                , "gray", "darkGray", "black", "red"
    ...
    public static void main(String[] args) {
        ...
        colorChooser.showDialog(null, "Color", Color.RED);
