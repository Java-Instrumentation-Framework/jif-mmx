...
// From: https://raw.githubusercontent.com/aeten/core/master/src/net.aeten.core.gui/net/aeten/core/gui/Colors.java
...
public class Colors {
   ...
   public static enum HSx {
      ...
      HSV {
                 @Override
                 public double[] rgb(double hue, double saturation, double value) {
                 ...
                 @Override
                 public double getLightness(double r, double g, double b) {
      ...
      HSL {
                 @Override
                 public double[] rgb(double hue, double saturation, double lightness) {
                 ...
                 @Override
                 public double getLightness(double r, double g, double b) {
                    return (StrictMath.max(r, StrictMath.max(g, b)) + StrictMath.min(r,
                            StrictMath.min(g, b))) / 2;
                 }

                 public double getSaturation(double r, double g, double b) {
                    double lightness = getLightness(r, g, b);
                    return lightness > 0.5 ? getChroma(r, g, b) / (2 - 2 * lightness) : getChroma(r,
                            g, b) / (2 * lightness);
                 }
              },
      HSI {
                 @Override
                 public double[] rgb(double hue, double saturation, double intensity) {
                 ...
                 @Override
                 public double getChroma(double r, double g, double b) {
                    return StrictMath.max(r, StrictMath.max(g, b)) - StrictMath.min(r,
                            StrictMath.min(g, b));
                 }
              },
      HSYp {
                 @Override
                 public double[] rgb(double hue, double saturation, double Yp) {
      ...
      public abstract double[] rgb(double h, double s, double x);
       ...
       */
      public abstract double getLightness(double r, double g, double b);

      public abstract double getSaturation(double r, double g, double b);

      public double getSaturationFromChroma(double chroma, double hue, double lightness) {
       ...
       */
      public double[] hcl(double r, double g, double b) {
         return new double[]{getHue(r, g, b),
            getChroma(r, g, b),
            getLightness(r, g, b)
         };
      }
       ...
       */
      public double[] hsx(double r, double g, double b) {
         return new double[]{getHue(r, g, b),
            getSaturation(r, g, b),
            getLightness(r, g, b)
         };
      ...
      public double[] rgbBest(double hue, double relativeToLightness) {
         ...
         double lightness = relativeToLightness > 0.5 ? 
                 (relativeToLightness - (relativeToLightness * optimalLightness)) : 
                 relativeToLightness + ((1.0 - relativeToLightness)
                 * optimalLightness);
      ...
      public double[][] rgbBest(double[] background, double rgbStartHue, int size) {
      ...
      private static double[] addM(double[] r1g1b1, double m) {
      ...
      public double getChromaFromHsl(double hue, double saturation, double lightness) {
         return saturation * getMaxChroma(hue, lightness);
      }

      public double getMaxChroma(double hue, double lightness) {
         double[] rgb = HSV.rgb(hue, 1, 1);
         double lightnessForMaxSaturation = getLightness(rgb);

         double rho, theta;
         if (lightness > lightnessForMaxSaturation) {
            rho = StrictMath.sqrt(1 + (1 - lightnessForMaxSaturation) * (1
                    - lightnessForMaxSaturation));
      ...
      private static double[] r1g1b1(double hue, double chroma) {
         double Hp = hue / 60;
